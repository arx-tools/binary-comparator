...
import { when } from '../node_modules/ramda/src/index.mjs'
...
(async () => {
  ...
  if (firstDeviationIdx === -1) {
    ...
    const char1 = file1[firstDeviationIdx]
    const char2 = file2[firstDeviationIdx]
    console.log(`  the two files differ at ${toHexIfNeeded(firstDeviationIdx)}: ${toHexIfNeeded(char1)} <> ${toHexIfNeeded(char2)}`)
  }
})()
